...
import type {
	...
	CatalogueExecutionStatus,
	CataloguePublicationStatus,
	CatalogueScope,
	CatalogueTrustStatus,
...
type WebDataset = {
	...
	latestRun: {
		id: string;
		executionStatus: CatalogueExecutionStatus;
		trustStatus: CatalogueTrustStatus | null;
...
function WebDatasetsBlock({ datasets }: { datasets: WebDataset[] }) {
	return (
		<Block>
			...
			<Span>
				These project datasets were generated by deterministic web robots and are only exposed after trust
				verification. Query their published files under <Bold>/datasets</Bold> with the local database. Before
				analysing a dataset, read its <Bold>README.md</Bold> trust provenance at the path below. Stay within the
				verified scope, distinguish source records, families, variants, and offers according to the dataset
				granularity, and mention material limitations when making catalogue-wide claims. Query only the{' '}
				<Bold>/latest</Bold> alias; never query <Bold>/versions</Bold> or rejected/diagnostic runs as current
				data.
			</Span>
			<List>
				{datasets.map((dataset) => (
					<ListItem key={dataset.slug}>
						...
						<Br />
						Scope: {dataset.activeConfiguration.scope.label} ({dataset.activeConfiguration.scope.entryUrl})
						<Br />
						{`Entities: ${dataset.activeRun.trustSummary?.entityCount ?? 0} ${dataset.activeRun.trustSummary?.granularity ?? 'entities'}`}
						<Br />
						Verification: {basisLabel(dataset.activeRun.trustSummary?.basis)}
						<Br />
						Verified: {dataset.activeRun.trustSummary?.verifiedAt ?? 'unknown'}
						<Br />
						Required coverage: {requiredCoverageLabel(dataset.activeRun.trustSummary)}
						<Br />
						Limitations: {limitationsLabel(dataset.activeRun.trustSummary)}
						<Br />
						Latest refresh: {latestRefreshLabel(dataset)}
					</ListItem>
				))}
			</List>
			<CodeBlock>
...
const basisLabel = (basis: CatalogueTrustSummary['basis'] | null | undefined): string =>
...
const percentageLabel = (value: number): string => `${Math.round(value * 100)}%`;

const requiredCoverageLabel = (summary: CatalogueTrustSummary | null): string => {
	if (!summary || summary.requiredCoverage.length === 0) {
		return 'none beyond base contract';
	}
	return summary.requiredCoverage
		.map(
			(entry) =>
				`${entry.concept} ${percentageLabel(entry.coverage)} (minimum ${percentageLabel(entry.minimumCoverage)})`,
		)
		.join(', ');
};

const limitationsLabel = (summary: CatalogueTrustSummary | null): string =>
	!summary || summary.limitations.length === 0 ? 'none recorded' : summary.limitations.join('; ');

const latestRefreshLabel = (dataset: WebDataset): string => {
	if (dataset.latestRun.id === dataset.activeRun.id) {
		return 'succeeded and published';
	}
	const statuses = `execution ${dataset.latestRun.executionStatus}; trust ${dataset.latestRun.trustStatus ?? 'none'}; publication ${dataset.latestRun.publicationStatus}`;
	return dataset.currentState.latestRefreshFailed ? `${statuses}; active data retained` : statuses;
};
